...
export default class YahooWeatherStore {
  ...
  load(location, callback) {
    ...
    let url = `https://query.yahooapis.com/v1/public/yql?format=json&q=select * from weather.forecast where woeid in (select woeid from geo.places(1) where text="${location}" limit 1) and u="${this.units}"`
    let weather = {
      success: false
    }
